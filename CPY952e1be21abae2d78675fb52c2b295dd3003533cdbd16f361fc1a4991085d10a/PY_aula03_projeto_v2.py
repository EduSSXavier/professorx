...
# 
# Este projeto é uma atualização da versão 1 
# da aplicação para cadastrar alunos, 
# Ele faz uso de funções para otimizar algumas
# partes do programa que anteriormente utilizavam
# apenas a estrutura while
# O programa armazena os dados em uma lista onde 
# cada elemento é um dicionário contendo informações 
# sobre um aluno.
# ------------------------------------------
...
def gerar_relatorio():
    for aluno in listaAlunos:
        media = (aluno['notas']['matematica'] + 
                 aluno['notas']['portugues'] + 
                 aluno['notas']['ciencias']) / 3
        if media >= 7:
            status = "Aprovado"
        else:
            status = "Reprovado"
